...
function checkStorageAndWarn() {
    ...
    indicator.innerHTML = `💾 ${usedMB.toFixed(1)}MB / 10MB`;
    ...
    if (percentUsed > 85) {
        ...
        warning.innerHTML = `
            ...
            <button onclick="quickBackupNow()" style="background:white; color:black; padding:10px 20px; margin:10px; border:none; border-radius:8px; cursor:pointer;">📥 BACKUP NOW</button>
...
function quickBackupNow() {
    ...
    alert('✅ Backup saved to Downloads/Files! Save this to iCloud or email to yourself.');
...
function showQuickBackup() {
    ...
    if (confirm(`📊 Storage: ${usedMB.toFixed(1)}MB / 10MB used\n\nClick OK to backup your data now.`)) {
        quickBackupNow();
    }
}
...
{ name: "C Mo:Mo (Veg)", price: 270, category: "C Mo:Mo", section: "Kitchen", type: "food", image: "images/momo_chicken.jpg" },
    ...
    { name: "Mocha", price: 320, category: "Frappe / Blended", section: "Bar", type: "drink", image: "images/blended.jpg" },
    { name: "Vanilla", price: 320, category: "Frappe / Blended", section: "Bar", type: "drink", image: "images/blended.jpg" },
    { name: "Caramel", price: 320, category: "Frappe / Blended", section: "Bar", type: "drink", image: "images/blended.jpg" },
    { name: "Hazelnut", price: 320, category: "Frappe / Blended", section: "Bar", type: "drink", image: "images/blended.jpg" },
    { name: "Oreo", price: 320, category: "Frappe / Blended", section: "Bar", type: "drink", image: "images/blended.jpg" },
...
const MAX_ITEMS_SOLD_ENTRIES = 300;
const STORAGE_AUTO_CLEAN_LIMIT_BYTES = 8 * 1024 * 1024; // ~8MB before aggressive trimming
...
function trimHistoryIfNeeded() {
    ...
    if (itemsSold.length > MAX_ITEMS_SOLD_ENTRIES) {
        itemsSold = itemsSold.slice(-MAX_ITEMS_SOLD_ENTRIES);
    }
...
function restoreData(file) {
    ...
    reader.onload = (e) => {
        try {
            ...
            discountCodes = data.discountCodes || discountCodes;
        ...
        try {
            ...
            if (voices.length > 0) {
                ...
                if (!selectedVoice && voices.length > 1) {
                    ...
                    console.log('⚠ No female voice found, using voices[1]:', selectedVoice.name);
    ...
    try {
        ...
        Object.entries(tableTimers).forEach(([table, timer]) => {
            if (orders[table] && orders[table].length > 0) {
                ...
                const previousElapsed = isNaN(timer.elapsed) ? 0 : (timer.elapsed || 0);
                const elapsed = previousElapsed + (now - lastUpdated);
                tableTimers[table] = { ...timer, elapsed: isNaN(elapsed) ? 0 : elapsed, lastUpdated: now };
                changed = true;
...
function navigateBackMenu() {
    ...
    if (searchInput.value.trim() !== '') {
        // If we're in search mode, just clear search and return to main menu
        searchInput.value = '';
        renderMenuNavigation();
        return;
    }
...
// Debounced renderOrderItems for performance
let renderTimeout;
function debouncedRenderOrderItems() {
    clearTimeout(renderTimeout);
    renderTimeout = setTimeout(() => renderOrderItems(), 100);
}
...
function showSalesReportsContent(e) {
    ...
    showSidebarContentModal('Sales Reports', `
        <div class="report-container">
            ...
            <button id="export-sales-pdf" class="export-btn" style="display:none; margin-top:10px; margin-left:10px;">Export to PDF</button>
        </div>
    `, () => {
    ...
    showSidebarContentModal('Order History', `
        <div class="report-container">
            ...
            <div class="report-filter" style="margin-top: 10px; margin-bottom: 5px; align-items: center;">
                ...
                <button id="show-all-history" class="export-btn" style="margin-left: 10px;">Show All</button>
            </div>
...
function renderOrderHistory(pageToShow, showAll = false) {
    try {
        const requestedPage = Number.isInteger(pageToShow) && pageToShow > 0 ? pageToShow : parseInt(pageToShow, 10) || 1;
        currentPage = requestedPage;
        showAllOrderHistory = Boolean(showAll);
    ...
    const paginatedHistory = showAllOrderHistory ? filteredHistory : paginateOrders(filteredHistory, currentPage);
...
function renderOrderHistoryPagination(totalOrders, page) {
    ...
    if (showAllOrderHistory) {
        const allInfo = document.createElement('span');
        allInfo.textContent = `Showing all ${totalOrders} records.`;
        allInfo.style.color = '#444';
        allInfo.style.alignSelf = 'center';
        pagination.appendChild(allInfo);

        if (totalOrders > itemsPerPage) {
            const pagedViewBtn = document.createElement('button');
            pagedViewBtn.textContent = 'Show paged view';
            pagedViewBtn.addEventListener('click', () => renderOrderHistory(1, false));
            pagination.appendChild(pagedViewBtn);
        }
        return;
    }
...
function renderVoidDetails() {
    try {
        ...
        if (!list) {
            console.error('void-details-list element not found');
            return;
        }
        ...
        console.log('Filtering void details. Start:', startDate, 'End:', endDate, 'Total records:', voidDetails.length);
        ...
        filteredVoids.forEach(voidItem => {
            ...
            tableHTML += `
                <tr>
                    ...
                    <td class="type-cell"><strong>${actionType}</strong></td>
...
function appendNumber(num) {
    ...
    appendNumberTimeout = setTimeout(updateChange, 50);
}
...
function renderPaymentMethods() {
    ...
    if (!paymentList.dataset.listenersAdded) {
        ...
        paymentList.addEventListener('touchstart', handlePaymentListTouch);
...
// Event delegation handler for payment list touches
function handlePaymentListTouch(event) {
    const button = event.target.closest('.remove-payment-btn');
    if (button) {
...
function updateChange() {
    ...
    if (completeBtn) {
        const effectivePaid = paidSoFar + Math.max(0, paymentAmount);
        const canComplete = total <= 0.01 || effectivePaid >= total - 0.01;
        completeBtn.disabled = !canComplete;
    }
    
    if (insufficientMsgEl) {
        if (remainingDue > 0.01 && total > 0.01) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("nav-kitchen-view")?.addEventListener("click", showKitchenView);
    ...
    const content = `
        <div class="kitchen-view">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <input type="text" id="kitchen-search" placeholder="Search table..." class="form-control" style="max-width: 200px;">
            </div>
            <div id="kitchen-orders">
                ${pendingKOTs.length === 0 ? '<p>No pending orders</p>' : 
                    pendingKOTs.map(kot => `
                        <div class="kot-card mb-3 p-3 border rounded">
                            <h5>Table ${kot.table} - ${kot.type}</h5>
                            <p><strong>KOT:</strong> ${kot.kotNumber}</p>
                            <p><strong>Items:</strong></p>
                            <ul>
                                ${kot.items.map(item => `<li>${item.name} x${item.quantity}</li>`).join('')}
                            </ul>
                            <div class="d-flex gap-2">
                                <button class="btn btn-success btn-sm" onclick="markKOTReady('${kot.kotId}')">Mark Ready</button>
                                <button class="btn btn-primary btn-sm" onclick="printKOTFromView('${kot.kotId}')">Print KOT</button>
                            </div>
                        </div>
                    `).join('')
                }
    ...
    showSidebarContentModal('Kitchen Display', content, () => {
        ...
        if (searchInput && ordersContainer) {
            searchInput.addEventListener('input', () => {
                ...
                cards.forEach(card => {
                    const tableText = card.querySelector('h5').textContent.toLowerCase();
                    const itemsText = Array.from(card.querySelectorAll('li')).map(li => li.textContent.toLowerCase()).join(' ');
                    const isVisible = !searchTerm || tableText.includes(searchTerm) || itemsText.includes(searchTerm);
        ...
        <div class="settings-panel">
            ...
            <div class="mb-3">
                <label class="form-check-label">
                    <input type="checkbox" class="form-check-input" id="sound-toggle" ${soundEnabled ? 'checked' : ''}>
                    Enable Sound Effects
                </label>
            </div>
            <div class="mb-3">
                <label class="form-check-label">
                    <input type="checkbox" class="form-check-input" id="speech-toggle" ${speechEnabled ? 'checked' : ''}>
...
async function mergeTables() {
    const confirmed = await showConfirmModal('Merge Tables', 
        'Merging tables will combine orders. This action cannot be undone. Continue?');
    if (!confirmed) return;
    
    const sourceTable = await showPromptModal('Merge Tables', 'Enter source table number to merge from:');
    if (!sourceTable || !tableList.includes(sourceTable)) {
        notifications.show('Invalid source table', 'error');
        return;
    }
    
    const targetTable = await showPromptModal('Merge Tables', `Merge Table ${sourceTable} into which table?`);
    if (!targetTable || !tableList.includes(targetTable)) {
        notifications.show('Invalid target table', 'error');
        return;
    }
    
    if (!orders[sourceTable]?.length) {
        notifications.show(`Table ${sourceTable} has no order to merge`, 'warning');
        return;
    }
    ...
    orders[sourceTable].forEach(item => {
        const existing = orders[targetTable].find(i => i.name === item.name && i.price === item.price);
        if (existing) {
    ...
    initializeTables();
    updateTotal();
    notifications.show(`Table ${sourceTable} merged into ${targetTable}`, 'success');
}
...
// Helper function for prompts - REMOVED: Using existing showPromptModal instead
...
function showHelpModal() {
    const content = `
        <div class="help-modal">
            ...
            <ul>
                <li><kbd>Ctrl+F</kbd> - Focus search bar</li>
                <li><kbd>F1</kbd> - Show this help</li>
                <li><kbd>Ctrl+Shift+C</kbd> - Open checkout</li>
                <li><kbd>Ctrl+Shift+M</kbd> - Merge tables</li>
            </ul>
            <h4>Tips</h4>
            <ul>
                <li>Use the sidebar for reports and utilities</li>
                <li>Search works across all menu items</li>
                <li>Tables show timers when occupied</li>
                <li>Use extras for item customization</li>
            </ul>
        </div>
    `;
